...
class Users extends BaseModel {
  ...
  registerUser = async (data: UserData): Promise<UserReturnData | null> => {
    ...
    return this.connection
      ...
      .catch((_) => {
        this.loggerManager.logWarn(`Register failed. Reason: Database refuse ` + _);
  ...
  loginUser = async (data: UserData): Promise<UserReturnData | null> => {
    const salt = await this.connection
      ...
      .catch((err) => {
        this.loggerManager.logWarn(`Login failed. Reason: Database refuse ` + err);
    ...
    return this.connection
      ...
      .then((result) => {
        ...
        this.loggerManager.logWarn(`Login failed. Reason: Invalid username or password.`);
  ...
  by = async (data: FilterData): Promise<UserReturnData[]> => {
    ...
    return this._transitionalVar.then((result) => {
      ...
      .catch((err) => {
        this.loggerManager.logWarn(`Get users failed. Reason: Database refuse ` + err);
        return [];
      });
  };
}
